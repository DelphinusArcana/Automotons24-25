...
public class ForwardTest extends LinearOpMode
{
    ...
    private double stoppedForwardPos;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (running) {
                ...
                telemetry.addData("Rotated", forwardDistance());
                power = 0.7;
            } else {
                telemetry.addData("Time elapsed", stoppedTime);
                telemetry.addData("Power", power);
                telemetry.addData("Rotated", stoppedForwardPos);
                power = 0.0;
            }
            ...
            if (gamepad1.y) {
                running = false;
                stoppedForwardPos = forwardDistance();
    ...
    public void forward (double power) {
        if (Math.abs(power) + maxAbsDrivePower() > power) {
            power = Math.signum(power) * (power - maxAbsDrivePower());
        }
